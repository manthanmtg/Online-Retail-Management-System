...
<html lang="en" dir="ltr">
    ...
    <body>
        <?php
            ...
            $column_headings = array("InvoiceNO", "OrderDate", "AmountPaid", "GSTIN");
            $sql = "SELECT ";
                ...
                if($value == "")
                    ...
                    if($key == "invcond") {
                        if(preg_match("/[0-9]*-[0-9]*/", $value)){
                            $arr = explode("-", $value);
                            if($arr[0]!="" and $arr[1]!="")
                                $where = $where."InvoiceNO>=".$arr[0]." AND "."InvoiceNO<=".$arr[1];
                            if($arr[0]=="" and $arr[1]!="")
                                $where = $where."InvoiceNO<=".$arr[1];
                            if($arr[0]!="" and $arr[1]=="")
                                $where = $where."InvoiceNO>=".$arr[0];
                            if($arr[0]=="" and $arr[1]=="")
                                $where = "";
                        }
                        else{
                            $where = $where."InvoiceNO=".$value;
                        }
                    }
                }
            }

...
<html lang="en" dir="ltr">
    ...
    <body>
        ...
        $column_headings = array("CustomerID", "EmailID", "Fname", "Minit", "Lname", "PhNo", "Address", "Pro", "RegDate", "Password");
        $sql = "SELECT ";
            ...
            if($value == "")
                ...
                if($key == "custidcond") {
                    if(preg_match("/[0-9]*-[0-9]*/", $value)){
                        $arr = explode("-", $value);
                        if($arr[0]!="" and $arr[1]!="")
                            $where = $where."CustomerID>=".$arr[0]." AND "."CustomerID<=".$arr[1];
                        if($arr[0]=="" and $arr[1]!="")
                            $where = $where."CustomerID<=".$arr[1];
                        if($arr[0]!="" and $arr[1]=="")
                            $where = $where."CustomerID>=".$arr[0];
                        if($arr[0]=="" and $arr[1]=="")
                            $where = "";
                    }
                    else{
                        $where = $where."CustomerID=".$value;
                    }
                }
            }
        }
